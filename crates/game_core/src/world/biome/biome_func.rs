    ...
    fn mats_for_site<'a>(
        ...
        if site_biome.stand_alone && s_site.is_finite() && s_site < SUB_COAST_LIMIT {
                ...
                pick_sub_biome_in_host(biomes, site_biome, site_pos, site_r, p_chunks, world_seed)
            {
                ...
                let adj = adjacency_support_factor(
                    biomes,
                    p_chunks,
                    world_seed,
                    site_biome,
...
pub fn site_properties_for_cell(
    ...
    let (area_min, area_max) = size_to_area_bounds(if biome.sizes.is_empty() {
        ...
        &biome.sizes
            [(rand_u32(cell_x, cell_z, (world_seed as u32) ^ SALT_PICK_SIZE) as usize)
            % biome.sizes.len()]
    });
    ...
    let jitter =
        0.95 + 0.10 * rand01(cell_x, cell_z, (world_seed as u32).wrapping_add(0xDEAD_BEEF));
...
#[inline]
pub fn get_biome_case_insensitive<'a>(
    biomes: &'a BiomeRegistry,
    name: &str,
) -> Option<&'a Biome> {
