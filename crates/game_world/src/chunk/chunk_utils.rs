...
use game_core::world::biome::BiomeStore;
use game_core::world::biome_plate::BiomePaletteSet;
...
pub async fn load_or_gen_chunk_async_biome(
    ws_root: PathBuf,
    coord: IVec2,
    store: Arc<BiomeStore>,
    palettes: BiomePaletteSet,
    cfg: WorldGenConfig,
) -> ChunkData {
    ...
    generate_chunk_async_biome(coord, store, palettes, cfg).await
}

pub async fn generate_chunk_async_biome(
    coord: IVec2,
    store: Arc<BiomeStore>,
    palettes: BiomePaletteSet,
    cfg: WorldGenConfig,
    ...
    // === Terrain noise setup (identical to your generate_chunk_async_noise) ===
    ...
    // Build a fast biome sampler once per chunk task.
    let sampler = store.make_sampler();
    ...
    for lx in 0..CX {
        for lz in 0..CZ {
            ...
            // Pick biome & palette once per column.
            let biome = sampler.biome_at_xz(wx, wz);
            let pal = palettes.get(biome);

            for ly in 0..CY {
                let wy = Y_MIN + ly as i32;
                if wy > h_final { break; }

                // Material selection now driven by palette + your terrain heuristics
                let id = if h_final >= SEA_LEVEL {
                    if r_here > 0.18 {
                        if wy == h_final { pal.beach_top }
                        else if wy >= h_final - 2 { pal.beach_top }
                        else if wy >= h_final - 3 { pal.filler }
                        else { pal.stone }
                    } else if near_coast && coast_mix {
                        if wy == h_final { pal.beach_top }
                        else if wy >= h_final - sand_under_sand { pal.beach_top }
                        else if wy >= h_final - sand_under_sand - dirt_after_sand { pal.filler }
                        else { pal.stone }
                    } else {
                        if wy == h_final { pal.top }
                        else if wy >= h_final - dirt_under_grass { pal.filler }
                        else { pal.stone }
                    }
                } else {
                    if r_here > 0.18 {
                        ...
                        if wy == h_final { pal.filler }
                        else if wy >= h_final - 1 { pal.filler }
                        else { pal.stone }
                    } else {
                        if wy == h_final { pal.seabed }
                        else if wy >= h_final - 3 { pal.seabed }
                        else { pal.stone }
                    }
                };
